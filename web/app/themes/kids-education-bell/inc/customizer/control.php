...
class Kids_Education_Bell_Dropdown_Chooser extends WP_Customize_Control{
	...
	public function render_content(){
		...
		?>
            <label>
                ...
                <select class="kids-education-bell-chosen-select" <?php $this->link(); ?>>
                    <?php
...
class Kids_Education_Bell_Dropdown_Multiple_Chooser extends WP_Customize_Control{
    ...
    public function render_content(){
        ...
        ?>
            <label>
                ...
                <select class="kids-education-bell-chosen-select" multiple <?php $this->link(); ?>>
                    <?php
                    foreach ( $this->choices as $value => $label )
                        echo '<option value="' . esc_attr( $value ) . '"' . selected( $this->value(), $value, false ) . '>' . esc_html( $label ) . '</option>';
                    ?>
...
Class Kids_Education_Bell_Customize_Horizontal_Line extends WP_Customize_Control {
    public $type = 'hr';

    public function render_content() {
        ?>
        <div>
            <hr style="border: 2px solid #6caf17;" />
        </div>
        <?php
    }
}

class Kids_Education_Bell_Multi_Input_Custom_Control extends WP_Customize_Control {
    /**
     * Control type
     *
     * @var string
...
class Kids_Education_Bell_Note_Control extends WP_Customize_Control {
