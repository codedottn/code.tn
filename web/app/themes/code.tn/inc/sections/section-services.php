 ...
 */
	...
	// Check if Polylang is installed and activated using the constant or main class
if ( defined( 'POLYLANG_VERSION' ) || class_exists( 'Polylang' ) ) {
	// Ensure the function exists just in case
	if ( function_exists( 'pll__' ) ) {
		$services_title = pll__( kids_education_bell_get_option( 'services_title' ), 'kids-education-bell' );
	} else {
		$services_title = kids_education_bell_get_option( 'services_title' );
	}
} else {
	// Fallback if Polylang is not available
	$services_title = kids_education_bell_get_option( 'services_title' );
}
