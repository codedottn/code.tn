...
<div class="project-slider modern-slider" data-slick='{"slidesToShow": 3, "slidesToScroll": 1, "infinite": true, "speed": 1200, "dots": 
<?php
if ( true == $project_dots ) {
	echo 'true';
} else {
	echo 'false'; }
?>
			...
			while ( $loop->have_posts() ) :
				...
				<article style="background-color: <?php echo esc_attr( $project_post_bg_color ); ?>">
					<div class="featured-image" style="background-image: url('<?php the_post_thumbnail_url( 'full' ); ?>');">
						<a class="post-thumbnail-link" disabled="true"></a>
					</div><!-- .featured-image -->
					<div class="entry-container <?php echo esc_attr( $content_align ); ?>">
						...
						<header class="entry-header">
							<h2 class="entry-title"><a disabled="true"><?php the_title(); ?></a></h2>
						</header>
						<div class="entry-content">
							...
							?>
							<?php
							$project_details = ! empty( kids_education_bell_get_option( 'project_lists_' . $i ) ) ? explode( '|', kids_education_bell_get_option( 'project_lists_' . $i ) ) : array();
